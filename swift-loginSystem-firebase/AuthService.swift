...
class AuthService {
    ...
    public func registerUser(
        with userRequest: RegisterUserRequest, completion: @escaping (Bool, Error?) -> Void
    ) {
        ...
        db.collection("users")
            ...
            .getDocument{ snapshot, error in
                ...
                if let snapshot = snapshot,
                   ...
                   let phone = snapshotData["phone"] as? Int,
                   let email = snapshotData["email"] as? String {
                    let user = User(username: username, email: email, userUID: userUID, phone: phone)
                    completion(user, nil)
                }
